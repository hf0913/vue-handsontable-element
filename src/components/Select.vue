<template>
    <div
        v-if="isRenderer"
        class="maple-select w-100 h-100 d-flex justify-center align-center"
        :id="`maple${row}${col}`"
    >
        <el-select
            size="mini"
    ...
    created() {
        if (this.col != null) {
            this.$store.commit("setKeepCellAttribute", {
                [`select-row-${this.row}-col-${this.col}`]: this.$store.state.MapleStore.changeCellProperties.bind(
                    this
                )
            });
            this.$store.commit("setChangeValue", {
                [`select-row-${this.row}-col-${this.col}`]: this.changeValue
            });
